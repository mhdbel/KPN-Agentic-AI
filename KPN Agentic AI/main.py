...
from graph.workflow import AgenticGraph, build_agentic_graph
from storage.persistence import PersistenceManager


class AgenticKPNChatbot:
    def __init__(self):
        self.graph: AgenticGraph = build_agentic_graph()
        self.persistence = PersistenceManager()
        print("🤖 Agentic KPN Chatbot initialized with persistence support!")

    def chat(self, query: str, thread_id: str = "default") -> str:
        previous_state = self.persistence.load_state(thread_id)
        response = self.graph.invoke(query, previous_state)

        serialisable_state = self._serialize_state(response)
        self.persistence.save_state(thread_id, serialisable_state)

        last_msg = response.get("messages", [])[-1] if response.get("messages") else ("assistant", "")
